...
- Sincronización best-effort con captura amplia de errores, lo que puede ocultar fallos de datos o conectividad.
...
- Falta validación visible de build release para iOS y Android en el repositorio.

## Mejoras recomendadas

- Restringir reglas Firestore por rol, colección y contexto de autenticación.
...
|---|---|---|---|---|---|---|---|---|---|---|---|---|
| BUG-001 | Reglas Firestore abiertas | Las reglas actuales permiten `read` y `write` a cualquier documento sin autenticación ni control de rol. | Exposición total de datos y manipulación remota. | P0 | Crítica | `firestore.rules` | 1. Revisar reglas actuales. 2. Intentar acceso a colecciones desde un cliente no autenticado. | Acceso denegado salvo permisos explícitos. | Acceso permitido globalmente. | Abierto | Pendiente de asignación | 2026-06-03 |
| BUG-002 | API key incrustada en el código | La creación de usuarios en el módulo admin usa una API key fija en `AdminRepository`. | Riesgo de fuga de credenciales y abuso de cuota. | P0 | Crítica | `lib/features/admin/services/admin_repository.dart` | 1. Inspeccionar el repositorio. 2. Localizar la key embebida. | Credenciales cargadas desde un secreto seguro. | La key está hardcodeada. | Abierto | Pendiente de asignación | 2026-06-03 |
| BUG-003 | Linux no configurado para Firebase | `DefaultFirebaseOptions.currentPlatform` lanza `UnsupportedError` para Linux. | La app no inicia en Linux. | P2 | Media | `lib/firebase_options.dart` | 1. Ejecutar la app en Linux. | Firebase inicializa o existe fallback documentado. | Se lanza error de plataforma no soportada. | Abierto | Pendiente de asignación | 2026-06-03 |
| BUG-004 | Sincronización sin trazabilidad de fallo | Varios métodos capturan errores remotos y continúan sin persistir causa ni contexto. | Dificulta soporte y diagnóstico. | P1 | Alta | `StudentApplicationRepository`, `CoordinatorRepository`, `AdminRepository` | 1. Forzar falla de Firestore. 2. Ejecutar sync. | Se registra el motivo y el contexto del error. | El error se ignora o se reduce a comentario. | Abierto | Pendiente de asignación | 2026-06-03 |
| BUG-005 | Validación de permisos centrada en cliente | El flujo de acceso depende de `currentUser.rol` y validaciones locales. | Riesgo de escalamiento de privilegios si el cliente es manipulado. | P0 | Crítica | `main.dart`, `access_policy.dart`, repositorios de rol | 1. Alterar estado local o payload remoto. 2. Navegar por rol. | Autorización reforzada por backend/reglas. | El control principal está en cliente. | Abierto | Pendiente de asignación | 2026-06-03 |
| BUG-006 | Carga documental sin adjunto real | El diálogo de documentos solicita solo el nombre del archivo, no un archivo o binario real. | Inconsistencia funcional si se esperaba gestión documental completa. | P2 | Alta | `lib/features/student/pages/student_application_detail_page.dart` | 1. Abrir solicitud en borrador. 2. Cargar documento. | Selección y almacenamiento real del archivo. | Solo se registra el nombre del archivo. | Abierto | Pendiente de asignación | 2026-06-03 |
| BUG-007 | Manejo de error confuso en login | El `catch` de `AuthService.login` usa nombres de parámetros invertidos y hace `print` de errores. | Depuración confusa y posible exposición de información en consola. | P2 | Media | `lib/features/auth/services/auth_service.dart` | 1. Forzar error en login. 2. Revisar logs. | Logs claros y sin datos sensibles. | Mensajes ambiguos y trazas impresas. | Abierto | Pendiente de asignación | 2026-06-03 |
| BUG-008 | Sin pipeline de build y release visible | No hay archivos de CI/CD en el repositorio para build, test y despliegue. | Riesgo operativo en liberaciones. | P2 | Media | Repositorio completo | 1. Revisar `.github/`, scripts y pipelines. | Existe pipeline reproducible. | No se evidencia pipeline de release. | Abierto | Pendiente de asignación | 2026-06-03 |
| BUG-009 | Validación de integridad remota parcial | La sincronización descarta conflictos y prioriza estado local sin resolución explícita. | Posibles desalineaciones entre local y Firestore. | P1 | Alta | `StudentApplicationRepository`, `CoordinatorRepository`, `AdminRepository` | 1. Modificar local y remoto simultáneamente. 2. Sincronizar. | Resolución determinística de conflictos. | La resolución no está definida explícitamente. | Abierto | Pendiente de asignación | 2026-06-03 |
| BUG-010 | Falta confirmación de build iOS/Android en repo | El proyecto incluye configuración nativa, pero no evidencia de validación automatizada de release. | Riesgo de ruptura en entrega. | P2 | Media | `android/`, `ios/` | 1. Ejecutar build release. 2. Revisar resultados. | Builds reproducibles y validados. | Pendiente de validación en el código fuente. | Abierto | Pendiente de asignación | 2026-06-03 |
