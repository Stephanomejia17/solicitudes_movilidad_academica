...
void main() {
  ...
  group('CoordinatorSolicitudDetailPage', () {
    ...
    testWidgets('muestra SnackBar de éxito tras aprobar correctamente', (tester) async {
      when(() => repository.watchSolicitud('s-1')).thenAnswer(
        (_) => Stream.value(_solicitud(id: 's-1', estado: 'enviada')),
      );
      when(() => repository.watchAprobaciones('s-1')).thenAnswer((_) => Stream.value(const <AprobacionData>[]));
      when(() => repository.approve(
            actor: coordinator,
            solicitudId: 's-1',
            comentario: '',
          )).thenAnswer((_) async {});
      ...
      await tester.pump();          // segundo StreamBuilder (watchAprobaciones)
    ...
    testWidgets('muestra SnackBar de error si repository.approve lanza una excepcion', (tester) async {
      when(() => repository.watchSolicitud('s-1')).thenAnswer(
        (_) => Stream.value(_solicitud(id: 's-1', estado: 'enviada')),
      );
      when(() => repository.watchAprobaciones('s-1')).thenAnswer((_) => Stream.value(const <AprobacionData>[]));
      ...
      // 1. Forzamos a Flutter a procesar el primer Stream (watchSolicitud)
      await tester.pump(Duration.zero); 
      
      // 2. Forzamos a Flutter a procesar el segundo Stream anidado (watchAprobaciones)
      await tester.pump(Duration.zero);
      expect(find.byType(CircularProgressIndicator), findsNothing);
      await tester.pumpAndSettle();
      // Esto imprimirá en la consola TODO lo que Flutter ve actualmente en pantalla
      ...
      expect(find.textContaining('Exception: fallo'), findsOneWidget);
    });
  });
}
